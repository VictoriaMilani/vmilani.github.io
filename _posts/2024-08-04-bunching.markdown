...
Notice there are 3 causal path to birth weights. The first is a direct path from education, another direct path from this $\eta$ variable, and another one, where cigarettes act as a _mediator variable_ between the explanatory, confounder, and the explained variable.
